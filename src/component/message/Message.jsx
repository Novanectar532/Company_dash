    ...
    <div className=" flex">
      <main className="flex  md:flex-row w-full h-screen">
        <div className="md:flex flex-col w-full md:w-1/2  border-r p-4 overflow-y-auto hidden ">
          ...
          <div className="flex flex-col gap-2">
            {messages.map((msg) => (
              <div
                key={msg.id}
                className={`flex items-center gap-3 p-3 rounded-lg cursor-pointer transition duration-300 ease-in-out ${selectedMessage.id === msg.id
                  ? "bg-indigo-100"
                  : "hover:bg-gray-100"
                  }`}
        ...
        <div className="flex flex-col w-full md:w-1/2 max-w-full px-6 md:py-6  py-2 overflow-y-auto">
          ...
          <div className={`fixed top-0 left-0 h-full  bg-gray-800 text-white bg-opacity-50 transition-all duration-500 ease-in-out ${isOpen ? "w-xsm" : "w-0"
            } overflow-hidden`}
          >
            <button
